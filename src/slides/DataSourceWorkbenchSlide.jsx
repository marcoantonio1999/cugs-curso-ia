...
import { classElevenSourceProfiles } from "../courseData";
import { accentText, accentizeDeep } from "../utils/accentText";

const sourceProfiles = accentizeDeep(classElevenSourceProfiles);

export default function DataSourceWorkbenchSlide() {
  const [profileIndex, setProfileIndex] = useState(0);
  const [stepIndex, setStepIndex] = useState(0);
  const activeProfile = sourceProfiles[profileIndex];
  const activeStep = activeProfile.steps[stepIndex];

  return (
    <div className="process-shell">
      <section className="process-profile-row" aria-label={accentText("Workbench de fuentes de datos de negocio")}>
      ...
      <section className="process-grid">
        ...
        >
          <div className="process-map-head">
            <div>
              <span className="mini-label">Data source workbench</span>
          ...
          >
            ...
            <div className="process-step-block">
              <span className="mini-label">{accentText("Que mirar primero")}</span>
        ...
        <article className="process-summary-card">
          <div className="process-map-head compact">
            <Database size={16} />
            <strong>{accentText("Lectura ejecutiva de la fuente")}</strong>
          </div>
