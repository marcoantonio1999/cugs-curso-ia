...
import { classSixteenExplorationProfiles } from "../courseData";
import { accentText, accentizeDeep } from "../utils/accentText";

const explorationProfiles = accentizeDeep(classSixteenExplorationProfiles);

export default function ExploratoryAnalysisWorkbenchSlide() {
  const [profileIndex, setProfileIndex] = useState(0);
  const [stepIndex, setStepIndex] = useState(0);
  const activeProfile = explorationProfiles[profileIndex];
  const activeStep = activeProfile.steps[stepIndex];

  return (
    <div className="process-shell">
      <section className="process-profile-row" aria-label={accentText("Workbench de analisis exploratorio")}>
        {explorationProfiles.map((profile, index) => (
          <button
            key={profile.id}
            type="button"
      ...
      <section className="process-grid">
        ...
        >
          <div className="process-map-head">
            <div>
              <span className="mini-label">EDA workbench</span>
        ...
        <article className="process-summary-card">
          <div className="process-map-head compact">
            <Search size={16} />
            <strong>{accentText("Lectura ejecutiva de exploracion")}</strong>
          </div>
