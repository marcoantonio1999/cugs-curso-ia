...
import { classSixProcessProfiles } from "../courseData";
import { accentText, accentizeDeep } from "../utils/accentText";

const processProfiles = accentizeDeep(classSixProcessProfiles);

export default function ProcessDesignWorkbenchSlide() {
  const [profileIndex, setProfileIndex] = useState(0);
  const [stepIndex, setStepIndex] = useState(0);
  const activeProfile = processProfiles[profileIndex];
  const activeStep = activeProfile.steps[stepIndex];

  return (
    <div className="process-shell">
      <section className="process-profile-row" aria-label={accentText("Procesos para disenar automatizacion")}>
        {processProfiles.map((profile, index) => (
          <button
            key={profile.id}
            type="button"
      ...
      <section className="process-grid">
        ...
        >
          <div className="process-map-head">
            <div>
              <span className="mini-label">Process workbench</span>
          ...
          >
            ...
            <div className="process-step-block">
              <span className="mini-label">{accentText("Mecanica recomendada")}</span>
        ...
        <article className="process-summary-card">
          <div className="process-map-head compact">
            <ShieldCheck size={16} />
            <strong>{accentText("Lectura ejecutiva del flujo")}</strong>
          </div>
          ...
          <div className="process-note-box">
            <span className="mini-label">Nota de diseño</span>
