...
export default function TabbedProcessWorkbenchSlide({ profiles, ariaLabel, headerLabel = "Workbench" }) {
  const [profileIndex, setProfileIndex] = useState(0);
  const [stepIndex, setStepIndex] = useState(0);
  const viewProfiles = accentizeDeep(profiles);
  const activeProfile = viewProfiles[profileIndex];
  const activeStep = activeProfile.steps[stepIndex];

  return (
    <div className="process-shell">
      <section className="process-profile-row" aria-label={accentText(ariaLabel)}>
      ...
      <section className="process-grid">
        ...
        >
          <div className="process-map-head">
            <div>
              <span className="mini-label">{accentText(headerLabel)}</span>
          ...
          >
            <div className="process-step-panel-head">
              <div>
                <span className="mini-label">{accentText("Paso seleccionado")}</span>
            ...
            <div className="process-step-block accent">
              <span className="mini-label">Guardrail</span>
        ...
        <article className="process-summary-card">
          <div className="process-map-head compact">
            <Compass size={16} />
            <strong>{accentText("Lectura ejecutiva")}</strong>
          </div>
